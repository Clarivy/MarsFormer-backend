...
def bake_normal(normal_input, mesh, resolution, normal_model):
    ...
    void main() {
        ...
        vec3 normal_model = normalize(v_normal);
        ...
        vec3 normal_tangent = normal_input*transpose(tbn);
        //normal_tangent=v_normal;
...
def main(mesh_path, normal_input_path, normal_model_path, output_path):
    ...
    # normal_input = normal_input[::4, ::4]
    # normal_model = zlw.read_normal(normal_model_path, use_xyz=True)[::-1]
    start = time.time()
    normal_baked = bake_normal(normal_input, mesh, (normal_input.shape[1], normal_input.shape[0]), None)
    # normal_baked[np.linalg.norm(normal_baked, ord=np.inf, axis=2) == 0] = [0, 0, 1]
    print(f"baked after {time.time() - start:.3f} s")
    zlw.save_normal(output_path, normal_baked, use_xyz=True, uint16=False)


if __name__ == "__main__":
    # mesh = read_obj(r"1013_normal\entity.obj")
    # normal_input = zlw.read_normal(r"1013_normal\src_1024.png", use_xyz=True)[::-1]
    # normal_model = zlw.read_normal(r"1013_normal\model_normal.png", use_xyz=True)[::-1]

    main(
        r"E:\20220427_metahuman_texutre_transfer\Metahuman_Head.obj",
        r"E:\20220427_metahuman_texutre_transfer\PNG\Ada_normal.png",
        # r"Y:\ProjectHH_Delivery\001\3_Texture_Package\process\00\UV_to_normal_u16.png",
        None,
        r"E:\20220427_metahuman_texutre_transfer\object_space.png",
    )
